...
function Main {
    ...
    # Honor the documented environment guard as well as the explicit switch.
    $effectiveDryRun = $DryRun -or $env:MOLE_DRY_RUN -eq "1"
    if ($effectiveDryRun) {
        $env:MOLE_DRY_RUN = "1"
    }
    else {
    ...
    # Run cleanup
    try {
        Start-Cleanup -IsDryRun $effectiveDryRun -IncludeSystem $System -IncludeGameMedia $GameMedia
    }
    finally {
        # Cleanup temp files
        Clear-TempFiles
